...
function EmailStateChanged() 
{
	...
	$main_content .= '<TR><TD width="150" valign="top"><B>Account name: </B></TD><TD colspan="2"><INPUT id="account_name" NAME="reg_name" onkeyup="checkAccount();javascript:somente_numero(this);" VALUE="" SIZE=30 MAXLENGTH=50><BR><font size="1" face="verdana,arial,helvetica">(Please enter your new account name)</font></TD></TR>
	...
	$main_content .= '</TABLE>
					  ...
					  <TR><TD>
					    ...
					    <TR><TD>
					      ...
					      <TEXTAREA ROWS="16" WRAP="physical" COLS="60" READONLY="true">';
...
if($action == "saveaccount")
{
	...
	elseif(!check_account_name($reg_name))
		$reg_form_errors[] = "Invalid account name format. Use only numbers 0-9.";
	if(empty($reg_email))
	...
	elseif(!$config['site']['create_account_verify_mail'])
	{
		if(!check_password($reg_password))
			$reg_form_errors[] = "Password contains illegal chars (0-9 only!) or lenght.";
	}
	//SECOND check
