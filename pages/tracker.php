...
if($logged)
{
    ...
    if($group_id_of_acc_logged >= $config['site']['access_admin_panel'] and $_REQUEST['control'] == "true")
    {
        ...
        if(!empty($_REQUEST['id']) and is_numeric($_REQUEST['id']) and !empty($_REQUEST['acc']) and is_numeric($_REQUEST['acc']))
        {
            ...
            {
                if($bug[2]['status'] == 2)
                    $value = "<font color=gray><b>WAITING</b></font>";
                elseif($bug[2]['status'] == 4)
                    $value = "<font color=green><b>SUPPORTED</b></font>";
                elseif($bug[2]['status'] == 3)
                    $value = "<font color=red><b>NOT A BUG</b></font>";
                elseif($bug[2]['status'] == 1)
                    $value = "<font color=#4169E1><b>NEW ANSWER</b></font>";
                ...
                $main_content .= '<TR BGCOLOR="'.$dark.'"><td width=40%><b>Subject:</b></td><td> '.$tags[$bug[2]['tag']].' '.$bug[2]['subject'].'  '.$value.'</td></tr>';    
                $main_content .= '<TR BGCOLOR="'.$light.'"><td><b>Type:</b></td><td>'.$valueType.'</td></tr>';    
                $main_content .= '<TR BGCOLOR="'.$dark.'"><td><b>Priority:</b></td><td> <img src=images/tracker/'.$bug[2]['priority'].'.png> '.$priority[$bug[2]['priority']].'';    
                $main_content .= '<TR BGCOLOR="'.$light.'"><td><b>Posted by:</b></td><td>';    
                foreach($players as $player)
                {
                    $main_content .= '<img src=images/tracker/t.png> '.$player->getName().'<br>';
                }
                $main_content .= '</td></tr>';
                $main_content .= '<TR BGCOLOR="'.$dark.'"><td colspan=2><b>Description:</b></td></tr>';    
                ...
                {
                    if($answer['who'] == 1)
                        $who = "<font color=red><b>SUPPORT</b></font>";
                    else
