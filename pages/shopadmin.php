...
function dropdown_offer_type($selected='item')
{
	...
	if($selected == 'item')
	{
		$return = '<select name="offer_type">
					...
					<option value="container">Scroll</option>
	...
	else if($selected == 'decoration')
	{
		$return = '<select name="offer_type">
					...
					<option value="decoration" selected="selected">Decoration</option>
					<option value="outfit">Outfit</option>
					<option value="container">Container</option>
				   </select>';
	}
	else if($selected == 'outfit')
	{
		$return = '<select name="offer_type">
					<option value="item">Item</option>
					<option value="scroll">Scroll</option>
		...
		case 'list':
			...
			$result .= '<tr bgcolor="#505050"><td class="white"><strong>Items cadastrados no \'Shop Offer\'</strong></td><tr>';
				...
				while($data = $ofertas->fetch())
				{
					...
					$result .= '<td align="center"><img src="' . $config['site']['item_images_url'] . $data['itemid1'] . $config['site']['item_images_extension'] . '" /></td>';
		...
		case 'points_add':
			...
			if(strlen($player) > 0){
				...
				if($data['account_id']){
					...
					$main_content .= '<strong class="success">Pontos adicionados com sucesso à: <i>'.$player.'</i>. Obrigado pelo apoio ao servidor!</strong><br /><br /><a href="?subtopic=shopadmin">Voltar</a>';
				}else{
